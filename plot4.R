...
png("plot4.png",width=4.8,height=4.8,units="in",res=100)
...
plot(power$dtime,power$Voltage,xaxt="n",type="n",
     xlab="datetime",ylab="Global active power (kilowatts)")
points(power$dtime,power$Voltage,type="l")
axis(1,at=c(min(power$dtime),median(power$dtime),max(power$dtime)),labels=c("Thu","Fri","Sat"))
...
plot(power$dtime,power$Global_reactive_power,xaxt="n",type="n",
     xlab="datetime")
points(power$dtime,power$Global_reactive_power,type="l")
axis(1,at=c(min(power$dtime),median(power$dtime),max(power$dtime)),labels=c("Thu","Fri","Sat"))
